/**
 * Based on code from https://gist.github.com/Warry/4254579.
 * Heavily modified for own requirements.
 */
...
(function ( $ ) {
  ...
  var scroll = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.msRequestAnimationFrame || window.oRequestAnimationFrame || false; //function(callback){ window.setTimeout(callback, 1000/60) };
  ...
  $.fn.parallaks = function(options) {
    ...
    if(scroll == false || typeof window.pageYOffset == 'undefined' || /MSIE (\d+\.\d+);/.test(navigator.userAgent)) {
  ...
  function initParallaxObjects() {
    ...
    originalObjects.each(function(index) {
      ...
      var originalWidth = $this.attr('data-originalwidth');
      var originalHeight = $this.attr('data-originalheight');
   ...
   * @return {[type]} [description]
   */
  getImgSizeData = function(injectedImgObject, cool, windowHeight, windowWidth) {
    ...
    if(newImgWidth > window.innerWidth) {
   ...
   */
  setParallaksData = function () {

    console.log('Executing setParallaksData()');
  ...
  function loop() {
    ...
    if (winScrollTop == window.pageYOffset) {
    ...
    console.log('winScrollTop 2: ' + winScrollTop);
    ...
    for (i =0; i<nrOfObjects; i++) {
      ...
      if ((winScrollTop >= injectedWrapperObjects[i].start - windowHeight && winScrollTop <= injectedWrapperObjects[i].stop)){
        ...
        setTop(injectedWrapperObjects[i].el, 0, (originalObjects[i].offsetTop - bodyPaddingTop) - (winScrollTop));

        setTop(injectedImgObjects[i].el, injectedImgObjects[i].positionX, (((originalObjects[i].offsetTop) - (winScrollTop))*injectedWrapperObjects[i].speed)*-1);
